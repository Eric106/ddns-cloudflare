...
class Cloudflare_DNS:
    ...
    def __post_init__(self):
        object.__setattr__(self,'cf',
                           CloudFlare(token=read_json('config/config.json')['cf_token']))
...
class Cloudflare_DDNS:
    zones : list[dict] = field(init=False)
    records_to_update : list[str] = field(init=False)

    def __post_init__(self):
        object.__setattr__(self,'zones',read_json('config/zones.json'))
    ...
    def update_ddns_records(self):
        ...
        cf_dns = Cloudflare_DNS(zones=self.zones)
